    ...
    <div class="row">
        <div class="col-md-12 col-sm-12 col-xs-12">
            <div class="detail-info pr-30 pl-30">
                <h3 class="title mb-3">{!! BaseHelper::clean($product->name) !!}</h3>

                <h5 class="title mb-3" id="quickViewPrice">{!! BaseHelper::clean($product->price) !!} TK</h5>
                        ...
                        <div class="product-extra-link2 @if (EcommerceHelper::isQuickBuyButtonEnabled()) has-buy-now-button @endif">
                            @if (EcommerceHelper::isCartEnabled())
                                <button type="submit" data-bs-dismiss="modal" aria-label="Close"
                                    ...
                                    @if ($product->isOutOfStock()) disabled @endif><i
                                        class="fi-rs-shopping-cart"></i>{{ __('Add to cart') }}</button>
                            @endif

                            @if (EcommerceHelper::isWishlistEnabled())
                                <a aria-label="{{ __('Add To Wishlist') }}"
                                    class="action-btn hover-up js-add-to-wishlist-button"
                                    data-url="{{ route('public.wishlist.add', $product->id) }}" href="#"><i
                ...
                <div class="font-xs">
                    ...
                    <ul class="mr-50 float-start">
                        ...
                        <li class="mb-5" @if (!$product->sku) style="display: none" @endif
                            id="product-sku">
                            <span class="d-inline-block">{{ __('SKU') }}</span>: <span
