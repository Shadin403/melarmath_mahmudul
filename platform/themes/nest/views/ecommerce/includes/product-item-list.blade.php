<div class="product-cart-wrap">
    <div class="product-img-action-wrap">
        ...
        <div class="product-action-1">
            <a aria-label="{{ __('Quick View') }}" href="#" class="action-btn hover-up js-quick-view-button"
    ...
    <div class="product-content-wrap">
        ...
        @endif
        <h2 class="text-truncate"><a href="{{ $product->url }}" title="{{ $product->name }}">{{ $product->name }}</a>
        </h2>
        @if ($product?->sub_title)
            <small class="text-truncate"
                style="font-size: 16px; color: #5a97fa; font-weight: bold;">{{ $product->sub_title }}</small>
        @endif
        ...
        <div class="mt-30 d-flex align-items-center">
            @if (EcommerceHelper::isCartEnabled())
                <a aria-label="{{ __('Add To Cart') }}" class="btn add-to-cart-button" data-id="{{ $product->id }}"
                    data-url="{{ route('public.ajax.cart.store') }}" href="#">
                    <i class="fi-rs-shopping-cart mr-5"></i>{{ __('Add to Cart') }}
                </a>
            @endif
            @if (EcommerceHelper::isCompareEnabled())
                <a aria-label="{{ __('Add To Compare') }}" href="#"
                    class="add-wishlish ml-30 text-body font-sm font-heading font-weight-bold js-add-to-compare-button"
                    data-url="{{ route('public.compare.add', $product->id) }}">
