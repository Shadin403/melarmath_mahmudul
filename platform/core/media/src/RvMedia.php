...
class RvMedia
{
    ...
    public function getImageUrl(
        ...
        if (
            ...
            if (! $this->isUsingCloud() && ! Storage::exists($thumbnailUrl)) {
                $this->generateThumbnailOnDemand($url, $size, $thumbnailUrl);
            }
    ...
    protected function generateThumbnailOnDemand(string $originalUrl, string $size, string $thumbnailUrl): bool
    {
        try {
            $realPath = $this->getRealPath($originalUrl);
            ...
            $dimensions = explode('x', $sizeValue);
            ...
            $result = Storage::put($thumbnailUrl, $image->encode(new AutoEncoder()));

            return $result;
        } catch (Throwable $exception) {
            logger()->error('Failed to generate thumbnail on demand: ' . $exception->getMessage(), [
