...
class OrderSupportServiceProvider extends ServiceProvider
{
    ...
    public function processPostCheckoutOrder(
        ...
        $shippingPrice = \Botble\Ecommerce\Models\GlobalOptionValue::where('option_id', 8)->first()?->affect_price;
        $success_url = url(route('public.checkout.success', $token) . '?payment=success');

        $failUrl = env('SOBKICHU_FAIL_URL');


        $cusName = $request->address['name'];
        $cusEmail = $request->address['email'];
        $apiKey = env('SOBKICHU_PAY_API');
        $systemUrl = config('app.url');


        if (isset($request->address['is_out_side_dhaka'])) {

            if ($request->address['is_out_side_dhaka'] != null) {
                if (isset($request->address['pay_delevery_charge'])) {
                    if ($request->address['pay_delevery_charge'] == 'full_payment') {
                        $processingAmount = $request->amount;
                    } else {
                        $processingAmount = $shippingPrice;
                    }
                } else {
                    $processingAmount = $shippingPrice;
                }
                ...
                $payload = [
                    ...
                    'webhook_url' => $systemUrl . "/callback/jonotapay?invoice=",
                ...
                $response = Http::withHeaders([
                    ...
                    'API-KEY' => $apiKey,
                ])->post('https://secure.sobkichubazar.com.bd/api/payment/create', $payload);
        ...
        } else if (isset($request->payment_method)) {
            if ($request->payment_method == 'sobkichubazarpay') {
                $payload = [
                    'cus_name' => $cusName,
                    'cus_email' => $cusEmail,
    ...
    public function handleCheckoutOrderByStore(
        ...
        if (isset($request->address['is_out_side_dhaka'])) {
            if ($request->address['is_out_side_dhaka'] != null) {
                $shippingAmount = \Botble\Ecommerce\Models\GlobalOptionValue::where('option_id', 8)->first()?->affect_price;
            }
        } else if (isset($request->address['is_inside_of_dhaka']) && isset($request->address['inside_dhaka'])) {
            $dhakaAreaCharge = DhakaArea::where('id', $request->address['inside_dhaka'])->first();
            if ($dhakaAreaCharge) {
                $shippingAmount = $dhakaAreaCharge->price;
            }
        }
        // $shippingAmount = isset($request->address['is_out_side_dhaka']) == null ? $shippingAmount : \Botble\Ecommerce\Models\GlobalOptionValue::where('option_id', 8)->first()?->affect_price;
    ...
    public function processGetCheckoutSuccess(string $token)
    {
        ...
        return view('plugins/marketplace::orders.thank-you', compact('orders', 'token'));
    }
    ...
    public function processPostSaveInformation(array $sessionCheckoutData, Request $request, string $token): array
    {
        ...
        $addressData['qr_code'] = 'processPostSaveInformation';
    ...
    public function handleProcessOrder(
        ...
        $addressData['qr_code'] = 'handleProcessOrder';
