    ...
    <div class="row">
        <div class="col-lg-7 col-md-6 col-12">
            ...
            <div class="d-flex gap-2" style="align-items: center;">
                <a class="btn payment-checkout-btn" href="{{ BaseHelper::getHomepageUrl() }}"> {{ __('Continue shopping') }}
                </a>
                <a class="btn btn-primary" id="invoice_download" href="{{ Route('generate-invoice', [292]) }}">
                    {{ __('Download Invoice') }}
                </a>
            </div>
            <style>
        ...
        <script>
            axios.get("{{ Route('getOrderUsingToken', [$token]) }}").then(response => {
                if (response.data.invoice && response.data.invoice.id) {
                    invoice_download.setAttribute('href',
                        `{{ url('generate-invoice') }}/${response.data.invoice.id}`)
                }
            })
        </script>
