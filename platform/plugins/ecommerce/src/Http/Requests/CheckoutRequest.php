...
class CheckoutRequest extends Request
{
    public function rules(): array
    {
        ...
        if (! auth('customer')->check()) {
            $rules = array_merge($rules, EcommerceHelper::getCustomerAddressValidationRules('address.'));
            $rules['address.email'] = ['nullable', 'email', 'max:60', 'min:6'];
            ...
            if ($this->input('address.email')) {
                $customer = Customer::query()->where('email', $this->input('address.email'))->first();
            }

            if (! $customer) {
                if ($this->input('address.email')) {
                    $rules['address.email'][] = Rule::unique((new Customer())->getTable(), 'email');
                }
