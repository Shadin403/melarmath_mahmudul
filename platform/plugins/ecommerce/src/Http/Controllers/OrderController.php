...
use App\Models\DhakaAreaTranslation;
...
use Botble\Ecommerce\Tables\InsideDhaka;
...
class OrderController extends BaseController
{
    ...
    public function store(CreateOrderRequest $request, CreatePaymentForOrderService $createPaymentForOrderService)
    {
        ...
        \Log::info('Order creation request data:', [
            ...
            'has_custom_prices' => $request->input('has_custom_prices', false)
        ]);
        ...
        if ($order) {
            ...
            if ($request->input('customer_address.name')) {
                OrderAddress::query()->create([
                    ...
                    'map_location' => $request->input('customer_address.map_location'),
                    'order_id' => $order->id,
                ]);
            } elseif ($customerId) {
            ...
            foreach (Arr::get($data, 'products') as $productItem) {
                ...
                // Use custom price if available, otherwise use original_price
                $itemPrice = Arr::get($productItem, 'price', Arr::get($productItem, 'original_price'));

                Log::info('Creating order product', [
                    'product_id' => Arr::get($productItem, 'id'),
                    'product_name' => Arr::get($productItem, 'name'),
                    'quantity' => $quantity,
                    'price_used' => $itemPrice,
                    'original_price' => Arr::get($productItem, 'original_price'),
                    'price_from_item' => Arr::get($productItem, 'price')
                ]);

                if ($product->is_variation && $product->variationInfo) {
                    $subtitle = $product->variationInfo->variation_title;
                } else {
                    $subtitle = $product->original_product->sub_title;
                }
                $orderProduct = [
                    'order_id' => $order->id,
                    'product_id' => Arr::get($productItem, 'id'),
    ...
    public function update(Order $order, UpdateOrderRequest $request)
    {
        ...
        // Check if we have product data (meaning this is a reorder update)
        if ($request->has('products') && is_array($request->input('products'))) {
            // Handle full product update (reorder scenario)
            $this->updateOrderWithProducts($order, $request);
        } else {
    ...
    protected function updateOrderWithProducts(Order $order, UpdateOrderRequest $request)
    {




        // Debug: Log the incoming request data for update
        ...
        if (Arr::get($data, 'error')) {
            return $this
                ->httpResponse()
                ->setError()
                ->setMessage($data['message'] ?? trans('plugins/ecommerce::order.error_during_creating_order'));
        }

        $currentShippingAmount = $order?->shipping_amount;


        // Update order basic information
        $order->fill($request->except([
            'customer_address.is_inside_of_dhaka',
            'customer_address.inside_dhaka',
        ]));

        // Fix: Ensure shipping_method is properly set from calculated data during reorder
        $order->shipping_method = 'default';



        $order->shipping_option = Arr::get($data, 'shipping_option');

        $order->sub_total = $request->sub_amount;
        $order->save();

        $address = $order->address;
        if ($address) {
            $address->is_inside_of_dhaka = $request->input('customer_address.is_inside_of_dhaka');
            $address->inside_dhaka = $request->input('customer_address.inside_dhaka');
            $address->save();
        };

        if ($currentShippingAmount != null && $request->customer_address['name']) {
            $order->shipping_amount = $currentShippingAmount;
            $order->update();
        }
        ...
        // Add new products with enhanced custom price support
        foreach (Arr::get($data, 'products') as $index => $productItem) {
            $productItem = $productItem->toArray($request);
            $quantity = Arr::get($productItem, 'quantity', 1);
            ...
            // Check for custom price from frontend request
            $requestProducts = $request->input('products', []);
            $customPrice = null;

            // Find custom price from request products array by index
            if (is_array($requestProducts) && isset($requestProducts[$index]['custom_price'])) {
                $customPrice = (float) $requestProducts[$index]['custom_price'];
            } elseif (is_array($requestProducts)) {
                // Try to find by product ID
                foreach ($requestProducts as $reqProduct) {
                    if (isset($reqProduct['id']) && $reqProduct['id'] == Arr::get($productItem, 'id') && isset($reqProduct['custom_price'])) {
                        $customPrice = (float) $reqProduct['custom_price'];
                        break;
                    }
                }
            }

            // Fallback to check in productItem itself
            if (!$customPrice && Arr::get($productItem, 'custom_price')) {
                $customPrice = (float) Arr::get($productItem, 'custom_price');
            }

            // Use custom price if available, otherwise use product price or original_price
            $itemPrice = $customPrice ?: (Arr::get($productItem, 'price', Arr::get($productItem, 'original_price')));

            $piece = Arr::get($productItem, 'options.piece', 0);
            if (!$piece && is_array($requestProducts) && isset($requestProducts[$index]['piece'])) {
                $piece = (float) $requestProducts[$index]['piece'];
            }

            Log::info('Updating order product', [
                'order_id' => $order->id,
                'product_id' => Arr::get($productItem, 'id'),
                'product_name' => Arr::get($productItem, 'name'),
                'quantity' => $quantity,
                'custom_price_detected' => $customPrice,
                'price_used' => $itemPrice,
                'piece' => $piece,
                'original_price' => Arr::get($productItem, 'original_price'),
                'price_from_item' => Arr::get($productItem, 'price'),
                'request_products_sample' => array_slice($requestProducts, 0, 2) // Log sample for debugging
            ]);
        ...
        $customerId = Arr::get($data, 'customer_id') ?: $request->input('customer_id');
        if ($customerId && $customerId != $order->user_id) {
            // Update the order's customer ID
            $order->user_id = $customerId;
            $order->save();

            Log::info('Updated order customer', [
                'order_id' => $order->id,
                'old_customer_id' => $order->user_id,
        ...
        if ($request->input('customer_address.name')) {
            // Get the current address values first
            $currentOrder = OrderAddress::query()->where('order_id', $order->id)->first();

            // Determine location type from request
            $isOutsideDhaka = $request->input('customer_address.is_out_side_dhaka', 0);

            // Calculate shipping amount based on location
            $newShippingAmount = 0;
            $oldShippingAmount = $order->shipping_amount;

            if ($isOutsideDhaka == 1) {
                // Outside Dhaka: use global option value for outside shipping
                $outsideShippingRate = \Botble\Ecommerce\Models\GlobalOptionValue::where('option_id', 8)->first();
                if ($outsideShippingRate) {
                    $newShippingAmount = $outsideShippingRate->affect_price;
                }
            } else {
                // Inside Dhaka: use DhakaArea price
                $insideDhakaArea = $request->input('customer_address.inside_dhaka');
                if ($insideDhakaArea) {
                    $dhakaArea = \App\Models\DhakaArea::where('id', $insideDhakaArea)->first();
                    if ($dhakaArea) {
                        $newShippingAmount = $dhakaArea->price;
                    }
                }
            }

            // Update order shipping amount and total amount with proper calculation
            $order->shipping_amount = $newShippingAmount;

            // Recalculate total amount properly to avoid calculation errors
            $subtotal = $order->sub_total ?: 0;
            $taxAmount = $order->tax_amount ?: 0;
            $discountAmount = $order->discount_amount ?: 0;
            $promotionAmount = $order->promotion_amount ?: 0;

            // Calculate new total: subtotal + tax - discounts + shipping



            $order->amount = max(($subtotal + $taxAmount - $discountAmount - $promotionAmount + $newShippingAmount), 0);

            $order->save();

            // Conditional data preparation based on location
            $addressData = [
                'name' => $request->input('customer_address.name'),
                'phone' => $request->input('customer_address.phone'),
                'email' => $request->input('customer_address.email'),
                ...
                'map_location' => $request->input('customer_address.map_location') ?: ($currentOrder->map_location ?? null),
                'qr_code' => $currentOrder->qr_code ?? null,
            ];

            if ($isOutsideDhaka == 1) {
                // Outside Dhaka: save state, city, courier_option; nullify inside Dhaka fields
                $addressData['state'] = $request->input('customer_address.state');
                $addressData['city'] = $request->input('customer_address.city');
                $addressData['courier_option'] = $request->input('customer_address.courier_option');
                $addressData['is_inside_of_dhaka'] = null;
                $addressData['inside_dhaka'] = null;
            } else {
                // Inside Dhaka: save inside Dhaka fields; nullify outside fields
                $addressData['is_inside_of_dhaka'] = $request->input('customer_address.is_inside_of_dhaka');
                $addressData['inside_dhaka'] = $request->input('customer_address.inside_dhaka');
                $addressData['state'] = null;
                $addressData['city'] = null;
                $addressData['courier_option'] = null;
            }

            // Delete existing order address
            $order->shippingAddress()->delete();
            $order->billingAddress()->delete();

            // Create new order address with conditional data
            OrderAddress::query()->create($addressData);

            Log::info('Updated order address with recalculated shipping amount', [
                'order_id' => $order->id,
                'is_out_side_dhaka' => $isOutsideDhaka,
                'old_shipping_amount' => $oldShippingAmount,
                'new_shipping_amount' => $newShippingAmount,
                'selected_area_id' => $request->input('customer_address.inside_dhaka'),
                'customer_name' => $request->input('customer_address.name'),
                'saved_fields' => $isOutsideDhaka ? 'state,city,courier_option' : 'is_inside_of_dhaka,inside_dhaka'
            ]);
        } elseif ($customerId) {
            ...
            if ($customer) {
                ...
                // Create new order address from customer data
                OrderAddress::query()->create([
                    'name' => $customer->name,
                    'phone' => $request->input('customer_address.phone') ?: $customer->phone,
                    'email' => $customer->email,
                ...
                Log::info('Updated order address from customer data', [
                    'order_id' => $order->id,
        ...
        // Check if order was updated and regenerate invoice if needed
        $hasCustomPrices = $request->input('has_custom_prices', false);
        $hasCustomerChanges = $request->input('customer_id') || $request->input('customer_address.name');
        $hadExistingInvoice = false;

        if ($order->invoice()->exists()) {
            $hadExistingInvoice = true;
            Log::info('Regenerating invoice for updated order', [
                'order_id' => $order->id,
                'invoice_id' => $order->invoice->id,
                'has_custom_prices' => $hasCustomPrices,
                'has_customer_changes' => $hasCustomerChanges
            ]);

            // Delete the existing invoice and its items to regenerate with updated data
            $existingInvoice = $order->invoice;
            $existingInvoice->items()->delete();
            $existingInvoice->delete();

            // Create a new invoice with updated prices
            InvoiceHelper::store($order);

            Log::info('Invoice regenerated successfully for order', [
                'order_id' => $order->id,
                'new_invoice_id' => $order->fresh()->invoice->id
            ]);
        }

        // Log the update
        Log::info('Order updated via reorder', [
            'order_id' => $order->id,
            'products_count' => count(Arr::get($data, 'products', [])),
            'amount' => $order->amount,
            'has_custom_prices' => $hasCustomPrices,
            'has_customer_changes' => $hasCustomerChanges,
            'invoice_regenerated' => $hadExistingInvoice
        ]);
    }
    ...
    public function getGenerateInvoice(Order $order, Request $request)
    {
        ...
        if (!$order->isInvoiceAvailable()) {
            // Only create invoice if order is not canceled
            if ($order->status != \Botble\Ecommerce\Enums\OrderStatusEnum::CANCELED) {
                InvoiceHelper::store($order);
                // Refresh the order to get the newly created invoice
                $order->refresh();
            } else {
                abort(404, 'Cannot generate invoice for canceled order');
            }
        }
    ...
    public function postUpdateShippingAddress(OrderAddress $address, AddressRequest $request)
    {
        ...
        $isOutside = $request->boolean('is_out_side_dhaka');
        $data['is_out_side_dhaka'] = $isOutside ? '1' : '0';

        if ($isOutside) {
            $data['is_inside_of_dhaka'] = null;
            $data['inside_dhaka'] = null;
        } else {
            $data['state'] = null;
            $data['city'] = null;
        }
        ...
        $currentOrder = Order::where('id', $address->order_id)->first();
        if ($currentOrder) {
            $newShippingAmount = 0;

            if ($address->is_out_side_dhaka) {
                $outsideShippingRate = GlobalOptionValue::where('option_id', 8)->first();
                if ($outsideShippingRate) {
                    $newShippingAmount = $outsideShippingRate->affect_price;
                }
            } elseif ($address->is_inside_of_dhaka && $address->inside_dhaka) {
                $dhakaArea = DhakaArea::where('id', $address->inside_dhaka)->first();
                if ($dhakaArea) {
                    $newShippingAmount = $dhakaArea->price;
                }
            }
            ...
            $promotionAmount = $currentOrder->promotion_amount ?: 0;

            // Calculate new total: subtotal + tax - discounts + shipping
            $currentOrder->shipping_amount = $newShippingAmount;
            $currentOrder->amount = max(($subtotal + $taxAmount - $discountAmount - $promotionAmount + $newShippingAmount), 0);
            $currentOrder->update();
        }
    ...
    {
        if (
            is_plugin_active('payment') && $request->input(
                'refund_amount'
    ...
    public function getReorder(Request $request)
    {
        ...
        foreach ($order->products as $orderProduct) {
            ...
            $options = [
                ...
                'piece' => $orderProduct->piece,
                'sub_title' => $product->getAttributes()['sub_title'], // ADD THIS LINE - get raw subtitle
            ];
        ...
        $customerAddress = new OrderAddressResource($order->address);
    ...
    protected function getDataBeforeCreateOrder(Request $request): array
    {
        ...
        foreach ($inputProducts as $inputProduct) {
            ...
            $piece = Arr::get($inputProduct, 'piece', 0);
            $options = [
                'name' => $productName,
                'image' => $image,
                'sub_title' => $parentProduct->sub_title,
            ...
            // Use custom price if provided, otherwise use product's front_sale_price
            $customPrice = Arr::get($inputProduct, 'custom_price') ?: Arr::get($inputProduct, 'price');
            if ($customPrice) {
                $price = (float) $customPrice;
                // Update the original_price in options to reflect the custom price
                $options['original_price'] = $price;
                Log::info('Using custom price for product', [
                    'product_id' => $productId,
                    'custom_price' => $customPrice,
                    'calculated_price' => $price,
                    'default_price' => $product->front_sale_price
                ]);
            } else {
                $price = $product->front_sale_price;
                $price = Cart::getPriceByOptions($price, $productOptions);
                Log::info('Using default price for product', [
                    'product_id' => $productId,
                    'default_price' => $product->front_sale_price,
                    'calculated_price' => $price
                ]);
        ...
        // Check if this is a reorder with inside Dhaka area selected
        $isInsideDhaka = $request->input('customer_address.is_inside_of_dhaka', 0);
        $insideDhakaAreaId = $request->input('customer_address.inside_dhaka');

        if ($isInsideDhaka && $insideDhakaAreaId) {
            // For inside Dhaka with area selected, use the area price
            $dhakaArea = DhakaArea::where('id', $insideDhakaAreaId)->first();
            if ($dhakaArea) {
                $shippingAmount = $dhakaArea->price;
                $shippingMethodName = $dhakaArea->name;
                $shippingMethod = 'dhaka_area';
                $shippingOption = $insideDhakaAreaId;
            }
        } elseif ($shippingType == 'free-shipping') {
    ...
    public function orderDetails(): mixed
    {
        Assets::addStylesDirectly(['vendor/core/plugins/ecommerce/css/ecommerce.css'])
            ->addScriptsDirectly([
                'vendor/core/plugins/ecommerce/libraries/jquery.textarea_autosize.js',
        ...
        $this->pageTitle(trans('Order details'));
        ...
        $orders = OrderProduct::whereBetween('created_at', [$from, $to])
            ->get()
            ->groupBy('product_id');
        ...
        return view('plugins/ecommerce::orders.details', compact('orders', 'defaultStore'));
    }




    public function downloadOrderSummaryPdf(Pdf $pdf)
    {
        $exclided = request('excluded_items');
        ...
        $rawOrders = OrderProduct::whereBetween('created_at', [$from, $to])
            ->get()
            ->groupBy('product_id');

        $orders = $rawOrders->map(function ($orderGroup) use ($notes) {
            return $orderGroup->map(function ($details) use ($notes) {
                $attributes = Arr::get($details->options, 'attributes', '');

                $weight = null;
                if (preg_match('/(?:Weight|⚖️ ওজন ⚖️|\*⚖ওজন⚖\*|ওজন)\s*:\s*([^),]+(?:\s*\(±\))?)/u', $attributes, $matches)) {
                    $weight = trim($matches[1]);
                }

                $optionValue = '';
                if (array_key_exists('optionCartValue', $details->product_options)) {
                    $optionArr = $details->product_options['optionCartValue'];
                    $firstKey = array_key_first($optionArr);
                    $optionValue = $optionArr[$firstKey][0]['option_value'] ?? '';
                }

                $note = $notes[$details->id] ?? null; // Merge note based on order_product ID
        ...
        // Generate PDF with proper language support and Twig extension
        return (new Pdf())
            ->templatePath(plugin_path('ecommerce/resources/templates/order-details.tpl')) // path to your tpl file
            ->destinationPath(storage_path('ecommerce/resources/templates/order-details.tpl'))
            ->supportLanguage(InvoiceHelper::getLanguageSupport())  // crucial for Bangla font support
            ->paperSizeA4()
            ->data(['orders' => $orders, 'excluded' => $exclided])
            ->twigExtensions([new TwigExtension()])
            ->setProcessingLibrary('mpdf')
            ->stream('order-summary.pdf');
    }

    public function insideDhaka(InsideDhaka $datatable)
    {
        $this->pageTitle('Inside Dhaka');
    ...
    public function createInsideOfDhaka()
    {
        $thana = GlobalOptionValue::where('option_id', 7)->get();
        return view('plugins/ecommerce::orders.create-area', compact(
            'thana'
        ));
    }

    public function storeInsideOfDhaka(Request $request)
    {
        $request->validate([
        ...
        $insideDhaka = InsideDhaka::create([
            'name' => $request->name,
            'description' => $request->description,
        ]);

        if ($request->hasFile('image')) {
            $imagePath = $request->file('image')->store('inside-dhaka-images', 'public');
            $insideDhaka->image = $imagePath;
            $insideDhaka->save();
        }

        return redirect()->route('admin.inside-dhaka.index')->with('success', 'Inside Dhaka created successfully.');
    }

    public function editInsideOfDhaka(InsideDhaka $insideDhaka)
    {
        $this->pageTitle('Edit Inside Dhaka');

        return view('admin.inside-dhaka.edit', compact('insideDhaka'));
    }

    public function updateInsideOfDhaka(Request $request, InsideDhaka $insideDhaka)
    {
        $request->validate([
            'name' => 'required|string|max:255',
            'description' => 'nullable|string',
        ...
        return redirect()->route('admin.inside-dhaka.index')->with('success', 'Inside Dhaka updated successfully.');
    }

    public function destroyInsideOfDhaka($id)
    {
        DhakaArea::findOrFail($id)->delete();
    ...
    public function saveDhakaArea(DhakaAreaRequest $request)
    {
        $thanaId = $request->input('thana_id');
        $areas = $request->input('areas', []);

        DB::transaction(function () use ($thanaId, $areas) {
            foreach ($areas as $areaData) {
                $dhakaArea = DhakaArea::create([
                    'name' => $areaData['name'],
                    'thana_id' => $thanaId,
                    'price' => $areaData['price'],
                ]);

                // Create translation record manually to avoid setAttribute issues
                $translation = new DhakaAreaTranslation();
                $translation->lang_code = 'en_US';
                $translation->ec_dhaka_area_id = $dhakaArea->id;
                $translation->name = null;
                $translation->save();
            }
        });

        // Check if user clicked "Save and Continue" button
        ...
        return $this
            ->httpResponse()
            ->setPreviousUrl(route('orders.inside-dhaka'))
            ->withCreatedSuccessMessage();
    }

    public function editDhakaArea($id)
    {
        $dhakaArea = DhakaArea::findOrFail($id);

        // Load translations
        $dhakaArea->load('translations');

        $thana = GlobalOptionValue::where('option_id', 7)->get();

        $this->pageTitle(trans('core/base::forms.edit_item', ['name' => $dhakaArea->name]));

        return view('plugins/ecommerce::orders.edit-area-form', compact(
            'dhakaArea',
            'thana'
        ));
    }

    public function updateDhakaArea(\Botble\Ecommerce\Http\Requests\DhakaAreaRequest $request, $id)
    {
        $dhakaArea = \App\Models\DhakaArea::with('translations')->findOrFail($id); // Make sure to use your correct model namespace

        // Handle language translations
        $currentLang = request()->get('ref_lang', 'bn');
        $isEnglishMode = str_starts_with($currentLang, 'en');

        // Debug: Log comprehensive request data
        \Illuminate\Support\Facades\Log::info('DhakaArea update comprehensive debug', [
            'id' => $id,
        ...
        if ($isEnglishMode) {
            // In English mode, we're updating the English translation
            $englishName = $request->input('name_en');

            \Illuminate\Support\Facades\Log::info('English mode processing', [
                'name_en' => $englishName,
                'currentLang' => $currentLang,
                'dhaka_area_id' => $dhakaArea->id,
            ]);

            if ($englishName) {
                // FIXED: Use explicit where conditions for composite keys instead of updateOrCreate
                $translation = DhakaAreaTranslation::where('ec_dhaka_area_id', $dhakaArea->id)
                    ->where('lang_code', $currentLang)
                    ->first();

                if ($translation) {
                    $translation->update(['name' => $englishName]);
                } else {
                    // FIXED: Create new translation using direct assignment to avoid setAttribute issues
                    $translation = new DhakaAreaTranslation();
                    $translation->ec_dhaka_area_id = $dhakaArea->id;
                    $translation->lang_code = $currentLang;
                    $translation->name = $englishName;
                    $translation->save();
                }

                // Debug: Log translation result
                \Illuminate\Support\Facades\Log::info('Translation updated/created successfully', [
                    'ec_dhaka_area_id' => $dhakaArea->id,
                    'lang_code' => $currentLang,
                    'name' => $englishName,
                ]);
            }

            // For English mode, we don't update the main record fields
            // They are preserved as hidden inputs in the form
        } else {
            // Default mode (e.g., 'bn') - update main record
            $data = $request->only('name', 'thana_id', 'price');
            $dhakaArea->update($data);

            \Illuminate\Support\Facades\Log::info('Default (Bengali) mode processing', [
                'updated_data' => $data
            ]);
        }
    ...
    public function deleteDhakaArea($id)
    {
        DhakaArea::findOrFail($id)->delete();
