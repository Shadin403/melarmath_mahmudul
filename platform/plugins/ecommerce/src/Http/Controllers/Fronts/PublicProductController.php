...
class PublicProductController extends BaseController
{
    ...
    public function getProductVariation(
        ...
        if ($attributes = $request->input('attributes', [])) {
            $cVariationInfo = ProductVariation::getVariationByAttributes($id, $attributes);
            if ($cVariationInfo) {
                (object) $product['variation_title'] = $cVariationInfo->variation_title;
                (object) $product['variation_desc'] = $cVariationInfo->variation_desc;
            }
        }
    ...
    public function getOrderTracking(OrderTrackingRequest $request)
    {
        ...
        if ($request->validated()) {
            ...
            // Handle cases where # in URL causes parsing issues
            if (!$code || $code === '#') {
                // Try to extract from the raw URL
                $fullUrl = request()->fullUrl();
                if (preg_match('/order_id=([^&]+)/', $fullUrl, $matches)) {
                    $code = $matches[1];
                }
            }

            // Clean the order code - remove # if present at the beginning
            $cleanCode = ltrim($code, '#');

            $query = Order::query()
                ->where(function (Builder $query) use ($code, $cleanCode): void {
                    $query
                        ->where('ec_orders.code', $code)           // Try with original format first
                        ->orWhere('ec_orders.code', '#' . $code)   // Try with # prefix
                        ->orWhere('ec_orders.code', $cleanCode)    // Try without # prefix
                        ->orWhere('ec_orders.code', '#' . $cleanCode); // Try clean code with # prefix
                })
