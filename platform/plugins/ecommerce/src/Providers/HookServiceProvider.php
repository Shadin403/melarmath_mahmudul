...
use Botble\Payment\Services\Gateways\SobkichubazarPayment;
...
class HookServiceProvider extends ServiceProvider
{
    public function boot(): void
    {
        ...
        // Add filter to override routes for DhakaArea model
        add_filter('route_actions', function (array $routes) {
            // Check if we're on the inside-dhaka page
            if (Route::currentRouteName() === 'orders.inside-dhaka') {
                // Override the edit route to use the correct one for DhakaArea
                $routes['edit'] = 'orders.editDhakaArea';
            }
        ...
        add_filter(FILTER_ECOMMERCE_PROCESS_PAYMENT, function (array $data, Request $request) {
            ...
            switch ($request->input('payment_method')) {
                ...
                case PaymentMethodEnum::SOBKICHU_PAY:

                    $data['charge_id'] = $this->app->make(SobkichubazarPayment::class)->execute($paymentData);
                    break;
                default:
