...
class EcommerceServiceProvider extends ServiceProvider
{
    ...
    public function boot(): void
    {
        ...
        if (defined('LANGUAGE_MODULE_SCREEN_NAME') && defined('LANGUAGE_ADVANCED_MODULE_SCREEN_NAME')) {
            ...
            LanguageAdvancedManager::registerModule(\App\Models\DhakaArea::class, [
                'name',
            ]);

            LanguageAdvancedManager::registerModule(ProductVariation::class, [
                'variation_title',
                'variation_desc',
            ]);

            LanguageAdvancedManager::addTranslatableMetaBox('product_options_box');
            // LanguageAdvancedManager::addTranslatableMetaBox('variations');
        ...
        DashboardMenu::beforeRetrieving(function (): void {
            DashboardMenu::make()
                ...
                ->registerItem([
                    'id' => 'cms-plugins-ecommerce-order-details',
                    'priority' => 10,
                    'parent_id' => 'cms-plugins-ecommerce',
                    'name' => 'Order details',
                    'icon' => 'ti ti-truck-delivery',
                    'url' => fn() => route('orders.order-details'),
                    'permissions' => ['orders.index'],
                ])
                ->registerItem([
                    'id' => 'cms-plugins-ecommerce-inside-dhaka',
                    'priority' => 10,
                    'parent_id' => 'cms-plugins-ecommerce',
                    'name' => 'Delivery Area Location',
                    'icon' => 'ti ti-location-pin',
                    'url' => fn() => route('orders.inside-dhaka'),
                    'permissions' => ['orders.index'],
                ])
                ->registerItem([
