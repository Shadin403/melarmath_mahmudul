...
class EcommerceHelper
{
    ...
    public function getCustomerAddressValidationRules(?string $prefix = ''): array
    {
        $rules = [
            ...
            $prefix . 'is_out_side_dhaka' => ['nullable', 'in:0,1,on,off'],
            $prefix . 'is_inside_dhaka' => ['nullable', 'integer', 'min:1'],
            $prefix . 'inside_dhaka' => ['nullable', 'integer', 'min:1'],
            $prefix . 'courier_option' => ['nullable', 'string', 'in:Sundorbon Courier,SA Paribahan', 'required_if:' . $prefix . 'is_out_side_dhaka,1'],
            $prefix . 'map_location' => ['nullable', 'url', 'max:255'],
        ];
    ...
    public function getProductsSearchBy(): array
    {
        ...
        if (empty($setting)) {
            return ['name', 'sku', 'description', 'slug'];
        }
