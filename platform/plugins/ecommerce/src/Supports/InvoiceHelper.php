...
class InvoiceHelper
{
    public function store(Order $order)
    {
        ...
        // Convert location IDs to names for invoice
        $isInsideDhaka = $address->is_inside_of_dhaka;
        $insideDhaka = $address->inside_dhaka;
        $isOutSideDhaka = $address->is_out_side_dhaka;

        // Convert IDs to names if they are numeric (indicating IDs instead of names)
        if (is_numeric($isInsideDhaka) && $isInsideDhaka) {
            // For thana, get the GlobalOptionValue by ID directly (matches PublicCheckoutController logic)
            $thanaModel = \Botble\Ecommerce\Models\GlobalOptionValue::find($isInsideDhaka);
            if ($thanaModel) {
                $isInsideDhaka = $thanaModel->option_value;
            }
        }

        if (is_numeric($insideDhaka) && $insideDhaka) {
            $areaModel = \App\Models\DhakaArea::with('translations')->find($insideDhaka);
            if ($areaModel) {
                // Use translation if available, otherwise use the name field
                $areaName = $areaModel->name;
                if ($areaModel->translations->where('lang_code', app()->getLocale())->first()) {
                    $areaName = $areaModel->translations->where('lang_code', app()->getLocale())->first()->name;
                }
                $insideDhaka = $areaName;
            }
        }
        ...
        $invoiceData = [
            ...
            'is_paid_delivery_charge' => $order->delivery_charge_status,
        ...
        foreach ($order->products as $orderProduct) {
            $invoice->items()->create([
                ...
                'product_sub_title' => $orderProduct->product_sub_title,
    ...
    public function getDataForInvoiceTemplate(Invoice $invoice): array
    {
        ...
        //     // options decode
        //     $decodedOptions = collect();
        //     if (!empty($item->options) && is_array($item->options)) {
        //         // if (isset($item->options['attributes'])) {
        //         //     $decodedOptions->push($item->options['attributes']);
        //         // }

        //         // if (isset($item->options['product_options'])) {
        //         //     $decodedOptions->push($item->options['product_options']);
        //         // }

        //         // if (isset($item->options['license_code'])) {
        //         //     $decodedOptions->push('License: ' . $item->options['license_code']);
        //         // }
        //         $decodedOptions->push([
        //             'optoins' => $item->options['product_options']
        //         ]);
        //     }
        ...
        //                 $decodedOptions->push($optionsArr['attributes']);
        //             }
        //             if (isset($optionsArr['product_options'])) {
        //                 $decodedOptions->push($optionsArr['product_options']);
        //             }
        //             if (isset($optionsArr['license_code'])) {
        //                 $decodedOptions->push('License: ' . $optionsArr['license_code']);
        //             }
        //         }
        //     }
