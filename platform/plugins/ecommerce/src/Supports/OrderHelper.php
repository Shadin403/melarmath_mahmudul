...
class OrderHelper
{
    ...
    public function processAddressOrder(int|string $currentUserId, array $sessionData, Request $request): array
    {
        $address = null;
        $qrCodeName = null;
        if (isset($request->address['map_location'])) {
            $mapLocationData = $request->address['map_location']; // Your custom text or URL
            $qrCodeSize = "300x300";

            $qrCodeData = Http::get("https://api.qrserver.com/v1/create-qr-code/?data=" . urlencode($mapLocationData) . "&size=$qrCodeSize");

            $qrCodeName = md5(rand(1, 100) . time()) . '.png';
            Storage::disk('public')->put($qrCodeName, $qrCodeData->body());
        }
        ...
        if (!empty($address)) {
            $addressData = [
                ...
                'is_out_side_dhaka' => $address->is_out_side_dhaka,
                'is_inside_of_dhaka' => $address->is_inside_of_dhaka,
                'road_name' => $address->road_name,
                'house_name' => $address->house_name,
                'map_location' => $address->map_location,
                'inside_dhaka' => $address->inside_dhaka,
                'qr_code' => $qrCodeName,
                'pay_delevery_charge' => $address->pay_delevery_charge,
                'courier_option' => $address->courier_option
            ];
        } elseif ((array) $request->input('address', [])) {
            $addressData = array_merge(
                ['order_id' => Arr::get($sessionData, 'created_order_id', 0), 'qr_code' => $qrCodeName],
            ...
            $addressData = [
                ...
                'order_id' => Arr::get($sessionData, 'created_order_id', 0),
                'is_out_side_dhaka' => Arr::get($sessionData, 'is_out_side_dhaka'),
                'inside_dhaka' => Arr::get($sessionData, 'inside_dhaka'),
                'is_inside_of_dhaka' => Arr::get($sessionData, 'is_inside_of_dhaka'),
                'road_name' => Arr::get($sessionData, 'road_name'),
                'house_name' => Arr::get($sessionData, 'house_name'),
                'map_location' => Arr::get($sessionData, 'map_location'),
                'qr_code' => $qrCodeName,
                'pay_delevery_charge' => Arr::get($sessionData, 'pay_delevery_charge'),
                'courier_option' => Arr::get($sessionData, 'courier_option')
            ];
        }

        return $this->checkAndCreateOrderAddress($addressData, $sessionData, $request);
    }

    public function checkAndCreateOrderAddress(array $addressData, array $sessionData, $requestData = null): array
    {
        ...
        if (!Arr::get($sessionData, 'is_save_order_shipping_address', true)) {
            ...
            if (!isset($sessionData['created_order_address'])) {
                if ($requestData != null) {
                    $createdOrderAddress = $this->createOrderAddress($addressData, $sessionData, $requestData);
                } else {
    ...
    protected function createOrderAddress(array $data, ?array $sessionData = [], $requestData = null): OrderAddress|bool
    {
        $data['type'] = OrderAddressTypeEnum::SHIPPING;

        if ($requestData != null) {
            if (isset($requestData->address['is_out_side_dhaka'])) {
                $data['is_out_side_dhaka'] = 1;
            } else if (isset($requestData->address['is_inside_of_dhaka'])) {
                if ($requestData->address['is_inside_of_dhaka']) {
                    $data['is_inside_of_dhaka'] = $requestData->address['is_inside_of_dhaka'];
                    $data['inside_dhaka'] = $requestData->address['inside_dhaka'];
                }
            }

            // Ensure map_location is included from request data
            if (isset($requestData->address['map_location'])) {
                $data['map_location'] = $requestData->address['map_location'];
            }
        }
    ...
    public function processOrderProductData(array|Collection $products, array $sessionData): array
    {
        ...
        if (!$createdOrderProduct || !$createdOrderProduct->eq($lastUpdatedAt)) {
            ...
            foreach ($cartItems as $cartItem) {
                ...
                if ($productByCartItem->is_variation && $productByCartItem->variationInfo) {
                    $subtitle = $productByCartItem->variationInfo->variation_title;
                } else {
                    $subtitle = $cartItem->options['sub_title'] ?? $productByCartItem->original_product->sub_title;
                }
                $data = [
