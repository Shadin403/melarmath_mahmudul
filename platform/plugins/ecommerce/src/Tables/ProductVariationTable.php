...
class ProductVariationTable extends TableAbstract
{
    ...
    public function htmlInitComplete(): ?string
    {
        return 'function (settings, json) {
            ...
            // Customize language column links for variations - show modal for specific variation
            $(settings.nTable).find("tbody tr").each(function() {
                ...
                row.find(".language-column a").each(function() {
                    var href = $(this).attr("href");
                    if (href && href.indexOf("?ref_lang=") > -1) {
                        // Change to custom modal trigger with variation ID
                        $(this).attr("href", "#");
