...
class ProductVariation extends BaseModel
{
    ...
    protected static function booted(): void
    {
        self::deleted(function (ProductVariation $variation): void {
            ...
            $variation->translations()->delete(); // Delete associated translations
    ...
    {
        return $this->hasMany(ProductVariationTranslation::class, 'ec_product_variations_id');
    }
    ...
    protected function variationTitle(): Attribute
    {
        return Attribute::get(function () {
            if (is_plugin_active('language-advanced') && LanguageAdvancedManager::isSupported($this)) {
                $currentLocale = Language::getCurrentLocaleCode();
                $defaultLocale = Language::getDefaultLocaleCode();

                if ($currentLocale !== $defaultLocale) {
                    $translation = $this->translations()->where('lang_code', $currentLocale)->first();
                    if ($translation && !empty($translation->variation_title)) {
                        return $translation->variation_title;
                    }
                }
            }

            return $this->getRawOriginal('variation_title');
        });
    }

    protected function variationDesc(): Attribute
    {
        return Attribute::get(function () {
            if (is_plugin_active('language-advanced') && LanguageAdvancedManager::isSupported($this)) {
                $currentLocale = Language::getCurrentLocaleCode();
                ...
                if ($currentLocale !== $defaultLocale) {
                    $translation = $this->translations()->where('lang_code', $currentLocale)->first();
                    if ($translation && !empty($translation->variation_desc)) {
                        return $translation->variation_desc;
                    }
                }
            }

            return $this->getRawOriginal('variation_desc');
        });
    }
}
