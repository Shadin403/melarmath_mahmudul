...
trait ProductActionsTrait
{
    ...
    public function postAddVersion(ProductVersionRequest $request, int|string|null $id, BaseHttpResponse $response)
    {
        ...
        if (! empty($addedAttributes) && is_array($addedAttributes)) {
            ...
            if (isset($result['variation'])) {
                $variationId = $result['variation']->id;
                $updateVariationInfo = ProductVariation::find($variationId);
                $updateVariationInfo->variation_title = $request->variation_title;
                $updateVariationInfo->variation_desc = $request->variation_desc;
                $updateVariationInfo->save();
            }
    ...
    public function postUpdateVersion(ProductVersionRequest $request, int|string $id, BaseHttpResponse $response)
    {
        ...
        $addedAttributes = $request->input('attribute_sets', []);
        $updateVariationInfo = ProductVariation::find($variation->id);
        $updateVariationInfo->variation_title = $request->variation_title;
        $updateVariationInfo->variation_desc = $request->variation_desc;
        $updateVariationInfo->update();
        if (! empty($addedAttributes) && is_array($addedAttributes)) {
        ...
        $availableProducts = Product::query()
            ...
            ->when($keyword, function ($query) use ($keyword): void {
                $query->where(function ($query) use ($keyword): void {
                    ...
                    // Enhanced search for both Bengali and English names
                    $query->where(function ($nameQuery) use ($searchKeyword, $keywordWildcard): void {
                        // Search in main product name (supports both Bengali and English)
                        $nameQuery->where('name', 'LIKE', $keywordWildcard);

                        // Also search using REGEXP for better Bengali/English matching
                        if (preg_match('/[\p{Bengali}]/u', $searchKeyword)) {
                            // If search contains Bengali characters, search in Bengali names
                            $nameQuery->orWhere('name', 'REGEXP', preg_quote($searchKeyword));
                        }

                        // Search in product translations (for multi-language support)
                        $nameQuery->orWhereHas('translations', function ($transQuery) use ($keywordWildcard, $searchKeyword): void {
                            $transQuery->where(function ($tq) use ($keywordWildcard, $searchKeyword): void {
                                $tq->where('name', 'LIKE', $keywordWildcard)
                                    ->orWhere('name', 'REGEXP', preg_quote($searchKeyword));
                            });
                        });
                    });

                    // SKU search (existing functionality)
                    $query->orWhere('sku', 'LIKE', $keywordWildcard)
                        ->orWhereHas('variations.product', function ($query) use ($keywordWildcard): void {
                            $query->where('sku', 'LIKE', $keywordWildcard);
                        });

                    // Enhanced slug search
                    $query->orWhereHas('slugable', function ($query) use ($searchKeyword, $keywordWildcard): void {
                        $query->where(function ($subQuery) use ($searchKeyword, $keywordWildcard): void {
                            $subQuery->where('key', 'like', $keywordWildcard)
                                ->orWhere('key', 'REGEXP', preg_quote($searchKeyword));
                        });
                    });

                    // Also search in slug translations
                    $query->orWhereHas('slugable.translations', function ($query) use ($searchKeyword, $keywordWildcard): void {
                        $query->where(function ($subQuery) use ($searchKeyword, $keywordWildcard): void {
                            $subQuery->where('key', 'like', $keywordWildcard)
                                ->orWhere('key', 'REGEXP', preg_quote($searchKeyword));
                        });
                    ...
                    // Search in product description (both Bengali and English)
                    $query->orWhere('description', 'LIKE', $keywordWildcard)
                        ->orWhereHas('translations', function ($transQuery) use ($keywordWildcard): void {
                            $transQuery->where('description', 'LIKE', $keywordWildcard);
                        });

                    // Search in product tags (if exists)
                    $query->orWhereHas('tags', function ($tagQuery) use ($keywordWildcard): void {
                        $tagQuery->where('name', 'LIKE', $keywordWildcard);
                    });

                    // Search in brand name (if exists)
                    $query->orWhereHas('brand', function ($brandQuery) use ($keywordWildcard): void {
                        $brandQuery->where('name', 'LIKE', $keywordWildcard)
                            ->orWhereHas('translations', function ($brandTransQuery) use ($keywordWildcard): void {
                                $brandTransQuery->where('name', 'LIKE', $keywordWildcard);
                            });
                    });
                });
            })
        ...
        $availableProducts = Product::query()
            ...
            ->when($keyword, function ($query) use ($keyword): void {
                $query->where(function ($query) use ($keyword): void {
                    ...
                    // Add slug search functionality
                    $query->orWhereHas('slugable', function ($query) use ($keyword): void {
                        $query->where(function ($subQuery) use ($keyword): void {
                            $subQuery->where('key', 'like', "%{$keyword}%");
                        });
                    });

                    // Also search in slug translations
                    $query->orWhereHas('slugable.translations', function ($query) use ($keyword): void {
                        $query->where(function ($subQuery) use ($keyword): void {
                            $subQuery->where('key', 'like', "%{$keyword}%");
                        });
                    });
