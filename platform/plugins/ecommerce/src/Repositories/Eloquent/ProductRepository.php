                ...
                $this->model = $this->model
                    ->where(function (EloquentBuilder $query) use ($keyword, $searchProductsBy, $isPartial): void {
                        ...
                        if (in_array('name', $searchProductsBy) || in_array('description', $searchProductsBy)) {
                            ...
                            // Fallback: also search base (default-locale) fields regardless of current locale
                            $query->orWhere(function (EloquentBuilder $subOuter) use ($keyword, $searchProductsBy, $isPartial): void {
                                $subOuter->where(function (BaseQueryBuilder $subQuery) use ($keyword, $searchProductsBy, $isPartial): void { // @phpstan-ignore-line
                                    if (in_array('name', $searchProductsBy)) {
                        ...
                        // Search by slug
                        if (in_array('slug', $searchProductsBy)) {
                            $function = $hasWhere ? 'orWhereHas' : 'whereHas';
                            $hasWhere = true;

                            $query->{$function}('slugable', function (EloquentBuilder $query) use ($keyword): void {
                                $query->where(function (BaseQueryBuilder $subQuery) use ($keyword): void { // @phpstan-ignore-line
                            ...
                            $query->orWhereHas('slugable.translations', function (EloquentBuilder $query) use ($keyword): void {
                                $query->where(function (BaseQueryBuilder $subQuery) use ($keyword): void { // @phpstan-ignore-line
                                    $subQuery->where('key', 'like', "%{$keyword}%");
                                });
                            });
                ...
                $this->model = $this->model
                    ->where(function (EloquentBuilder $query) use ($keyword, $searchProductsBy, $isPartial): void {
                        ...
                        if (in_array('name', $searchProductsBy) || in_array('sku', $searchProductsBy) || in_array('description', $searchProductsBy)) {
                            ...
                            // Also search translations (any locale) to cover cases where default-locale data is stored in translations
                            $query->orWhereHas('translations', function (EloquentBuilder $tq) use ($keyword, $searchProductsBy, $isPartial): void {
                                $tq->where(function (BaseQueryBuilder $subQuery) use ($keyword, $searchProductsBy, $isPartial): void { // @phpstan-ignore-line
                                    if (in_array('name', $searchProductsBy)) {
                                        $subQuery->addSearch('name', $keyword, $isPartial);
                                    }
