...
class StoreProductService
{
    public function execute(Request $request, Product $product, bool $forceUpdateAll = false)
    {
        ...
        $slug = Slug::where('reference_id', $product->id)->first();

        if ($request->slug == $slug->key) {
            $purifySlug = $request->slug;
        } else {
            if (Slug::where('key', $request->slug == (null || '') ? $product->name : $request->slug)->get()->count() > 0) {
                $purifySlug = $request->slug == (null || '') ? $product->name : $request->slug . '-' . Slug::where('key', $request->slug == (null || '') ? $product->name : $request->slug)->get()->count();
            } else {
                $purifySlug = $request->slug == (null || '') ? $product->name : $request->slug;
            }
        }
