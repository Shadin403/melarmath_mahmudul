...
@section('content')
    <div class="max-width-1200" id="main-order">
        <create-order :order_id="{{ $order->id }}" :products="{{ json_encode($products) }}"
            :product_ids="{{ json_encode($productIds) }}" :customer="{{ $customer ? $customer : null }}"
            :customer_id="{{ $order->user_id }}" :customer_addresses="{{ json_encode($customerAddresses) }}"
            :customer_address="{{ $customerAddress->toJson() }}" :sub_amount="{{ $order->amount }}"
            ...
            :is_selected_shipping="true" :customer_order_numbers="{{ $customerOrderNumbers }}"
            ...
            :sub_amount_label="'{{ format_price(0) }}'" :tax_amount_label="'{{ format_price(0) }}'"
            :promotion_amount_label="'{{ format_price(0) }}'" :discount_amount_label="'{{ format_price(0) }}'"
            :shipping_amount_label="'{{ format_price(0) }}'" :total_amount_label="'{{ format_price(0) }}'"
            :payment-methods="{{ json_encode(is_plugin_active('payment') ? \Botble\Payment\Enums\PaymentMethodEnum::labels() : []) }}"
            :payment-statuses="{{ json_encode(is_plugin_active('payment') ? \Botble\Payment\Enums\PaymentStatusEnum::labels() : []) }}"></create-order>
    </div>
@stop
