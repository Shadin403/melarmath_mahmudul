    ...
    <x-core::table.body>
        ...
        <x-core::table.body.row>
            ...
            <x-core::table.body.cell>
                {{ number_format($order->products->sum('qty'), 0) }}
            </x-core::table.body.cell>
        </x-core::table.body.row>
        <x-core::table.body.row>
            <x-core::table.body.cell>Piece/wieght</x-core::table.body.cell>
            <x-core::table.body.cell>
                {{ number_format($order->products->sum('piece'), 3) }}
            </x-core::table.body.cell>
        </x-core::table.body.row>
        <x-core::table.body.row>
            <x-core::table.body.cell>
        ...
        @if (is_plugin_active('payment') && $order->payment->id)
            ...
            <x-core::table.body.row>
                <x-core::table.body.cell>
                    {{ trans('Advance (Courier Charge)') }}
                </x-core::table.body.cell>
                <x-core::table.body.cell>
                    @if ($isInAdmin && $order->payment->id)
                        <span>{{ format_price($order->payment->advance_payment != null ? $order->payment->advance_payment : 0) }}</span>
                    @else
                        <span>{{ format_price($order->payment->advance_payment != null ? $order->payment->advance_payment : 0) }}</span>
                    @endif
                </x-core::table.body.cell>
